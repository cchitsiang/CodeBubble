...
private let sidebarGroups: [SidebarGroup] = [
    SidebarGroup(title: nil, pages: [.general, .behavior, .appearance, .mascots, .sound, .shortcuts, .about]),
]
...
private struct AppearancePreview: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 8) {
            // Column 1: Mascot
            VStack(spacing: 3) {
                MascotView(source: "claude", status: .thinking, size: 32)
                if showDetails {
                    HStack(spacing: 1) {
...
private struct MascotsPage: View {
    @ObservedObject private var l10n = L10n.shared
    @State private var previewStatus: AgentStatus = .thinking
        ...
        Form {
            Section {
                Picker(l10n["preview_status"], selection: $previewStatus) {
                    Text(l10n["processing"]).tag(AgentStatus.thinking)
                    Text(l10n["idle"]).tag(AgentStatus.idle)
                    Text(l10n["waiting_approval"]).tag(AgentStatus.waitingForUser)
                }
    ...
    var body: some View {
        VStack {
            ...
            VStack(spacing: 24) {
                ...
                HStack(spacing: 12) {
                    aboutLink("GitHub", icon: "chevron.left.forwardslash.chevron.right", url: "https://github.com/cchitsiang/CodeBubble")
                    aboutLink("Issues", icon: "ladybug", url: "https://github.com/cchitsiang/CodeBubble/issues")
                }
...
struct AppLogoView: View {
    ...
    var body: some View {
        Group {
            if let url = Bundle.appModule.url(forResource: "logo", withExtension: "png"),
                ...
                Section {
                    VStack(alignment: .leading, spacing: 8) {
                        HStack(spacing: 6) {
                            Image(systemName: "exclamationmark.triangle.fill")
                                .foregroundStyle(.orange)
                            Text(l10n["shortcut_accessibility_title"])
                                .font(.headline)
                        }
                        Text(l10n["shortcut_accessibility_desc"])
                            .font(.caption)
                            .foregroundStyle(.secondary)
                        Button(l10n["shortcut_accessibility_open"]) {
