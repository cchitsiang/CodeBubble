...
private let log = Logger(subsystem: "com.codebubble", category: "Panel")
...
class PanelWindowController: NSObject, NSWindowDelegate {
    ...
    func showPanel() {
        ...
        sessionObservationTask = Task { @MainActor [weak self] in
            while !Task.isCancelled {
                withObservationTracking {
                    ...
                    _ = self?.appState.totalSessionCount
                } onChange: {
        ...
        globalClickMonitor = NSEvent.addGlobalMonitorForEvents(matching: [.leftMouseDown, .rightMouseDown]) { [weak self] event in
            Task { @MainActor in
                ...
                // Don't close during completion card
                switch self.appState.surface {
                case .completionCard: return
                default: break
                }
    ...
    func isActiveTerminalForeground() -> Bool {
        guard let sessionId = appState.activeSessionId,
              let session = appState.sessions[sessionId] else { return false }
